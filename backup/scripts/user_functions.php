...
function ShowCategories($limit="",$recordperpage=""){
	...
	?>
      <ul class="circle category-list clearfix">
							...
							?>
				              <li><a href="departments?courses=<?php echo $category['cat_id'];?>"><?php echo $category['cat_name'];?></a><span class="posts-count"><?php echo CountItem($category['cat_id'],'department');?></span></li>
                              <?php
							  }
							  ?>
				            </ul>
...
function ShowArticles($limit="",$recordperpage="",$single=false){
	...
	ob_start();
	if(!$single){
	$article = $database -> select(true,"articles","","sort_date desc ".$limitQuery);
							...
							<div class="feature-box clearfix">
				                ...
				                <div class="feature-box-content">
					                ...
					                <p><?php echo RemoveHTML($article['article_content'], 200);?></p>
                                     ...
                                     <p><a href="article?detail=<?php echo $article['article_id'];?>" class="read-more" style="color:#9B2231;">Article Detail<i class="fa fa fa-long-arrow-right"></i></a></p>
					            </div>
				            </div>
     <?php  
							}
		...
		$article = $database -> select(false,"articles","","sort_date desc ");
		?>
          <strong><?php echo changeCase($article['article_title'],'s');?></strong>
          <div>
          <?php echo RemoveHTML($article['article_content'], 400);?>....
          </div>
          <a href="article?detail=<?php echo $article['article_id'];?>" class="read-more">Read More&nbsp;<i class="fa fa fa-long-arrow-right"></i></a>
        <?php
	}
	$content = ob_get_clean();
	return $content;
