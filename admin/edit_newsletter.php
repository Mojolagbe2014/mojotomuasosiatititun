...
if(isset($_POST['sbmt']) &&$_POST['sbmt'] == 1){
	...
	if(MysqlQuery("update newsletter set newsletter_title='".$article['title']."',newsletter_content='".$article['description']."' where newsletter_id='".$_GET['id']."'"));
	$result = MysqlSelectQuery('select * from subscribers');
		//sending out mails
		while($rows = SqlArrays($result)){
			$to = $rows['emails'];
		$subject = $article['title'];
		$innerMsg = $article['description'];
		SendHtmlEmails($to,$subject,$innerMsg);
		}
	header("location: edit_newsletter.php?status=success&edit=true&id=".$_GET['id']);
...
if(isset($_GET['id']) && $_GET['edit'] == 'true'){
	$result = MysqlSelectQuery("select * from newsletter where newsletter_id='".$_GET['id']."'");
...
<title>Edit Newsletter | Admin</title>
         ...
         <div class="container">
            <ul>
                      ...
                      <li><a href="newsletter_new.php" ><span>New Newsletter</span></a></li>
                        <li><a href="#" class="current"><span>Edit Newsletter</span></a></li>
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
     <h1 class="content_edit">Edit Newsletter</h1>
    </div>
    ...
    <div class="portlet">
    	...
    	<label>Subject</label>
        <input name="title" type="text" class="smallInput wide" id="title" value="<?php echo $rows['newsletter_title'];?>" />
        
        <label>Content</label>
        <textarea name="description" cols="30" rows="7" class="smallInput wide" id="description"><?php echo $rows['newsletter_content'];?>
        </textarea>
        <script type="text/javascript">
        ...
        <!-- BUTTONS -->
        <a class="button" href="javascript:void(0);" onclick="Send()"><span>Resend Newsletter</span></a>
