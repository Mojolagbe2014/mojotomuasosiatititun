...
MysqlQuery("insert into quotes(content,posted_date)
										  values('".$_POST['content_quote']."','".date('Y-m-d')."')");
header("location:./newquote.php?status=success");
}
...
<!-- TABS START -->
    <div id="tabs">
         <div class="container">
            <ul>
                       <li><a href="quotes.php"><span>All quotes</span></a></li>
                      <li><a href="newquote.php" class="current"><span>New Quote</span></a></li>
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
     <h1 class="content_edit">New Quote</h1>
    </div>
    ...
    <div class="portlet">
		<form id="quotes" action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
		  <label>Todays Quote</label>
        <textarea name="content_quote" cols="30" rows="7" class="smallInput wide" id="content_quote"></textarea>
      <input name="sbmt_quote" type="hidden" value="" id="sbmt_quote" />
        <!-- BUTTONS -->
       <a class="button" href="javascript: void(0);"  onclick="Send('sbmt_quote')"><span>Submit Quote</span></a>
      </form>
      </div>
<!--  END #PORTLETS -->  
   </div>
