...
$result = MysqlSelectQuery("select * from subscribers order by name limit $offset, $recordperpage");

if(isset($_GET['delete'])){
	MysqlQuery("delete from subscribers where subscribers_id='".$_GET['delete']."'");
	header("location: ".$_SERVER['PHP_SELF']);
}

if(isset($_POST['deleteLink']) && $_POST['deleteLink'] == 1){
	if(!isset($_POST['checkboxsubscriber'])){
		 $message = errorMsg("You did not select any subscriber, Please select at least one subscriber to delete!");
	 }
	 else{
	while(list($key,$val) = each($_POST['checkboxsubscriber'])){
		MysqlQuery("DELETE FROM subscribers WHERE subscribers_id ='".$_POST['checkboxsubscriber'][$key]."'");	
			}
...
<!-- TABS START -->
    <div id="tabs">
         <div class="container">
            <ul>
                      <li><a href="subscribers.php" class="current"><span>Subscribers</span></a></li>
                      <li><a href="new_subscribers.php" ><span>New</span></a></li>
...
<!-- CONTENT START -->
    ...
    <!--RIGHT TEXT/CALENDAR-->
    <div class="grid_6" id="eventbox"><a href="#" class="inline_calendar">You have a total of (<?php echo NUM_ROWS($result);?>) Subscribers</a>
    ...
    <div class="portlet">
        ...
        <form action="" method="post" id="eventsForm">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" >
            <thead>
              <tr id="chkrow">
                <th width="77" scope="col"><input name="checkall" type="checkbox" id="checkall" value="" /></th>
                <th width="357" scope="col">Name</th>
                <th width="357" scope="col">Email</th>
                <th width="125" colspan="-2" align="center" scope="col">Actions</th>
              </tr>
            </thead>
            <tbody>
            <?php
			...
			?>
              <tr>
                <td width="77"><label>
                  <input type="checkbox" name="checkboxsubscriber[]" id="checkbox" value="<?php echo $rows['subscribers_id'];?>" />
                </label></td>
                <td><?php echo $rows['name'];?></td>
                <td><?php echo $rows['emails'];?></td>
                <td width="125" colspan="-2" align="left"> <a href="javascript:Deletesubscriber(<?php echo $rows['subscribers_id'];?>)" class="delete_icon" title="Delete"></a></td>
              </tr> 
              ...
              <tr class="footer">
                <td><a href="javascript:void(0);" class="delete_inline" onclick="Send('delete')">Delete all</a></td>
                <td colspan="4" align="right">
                  <?php Pages_admin("SELECT COUNT(subscribers_id) AS numrows FROM subscribers ",$recordperpage,$pagenum,"subscribers.php?get"); ?>                </td>
                </tr>
                <?php
			}
              else{
			  ?>
              <tr>
                <td colspan="4" align="center"><?php echo warningMsg("No subscriber(s) found");?></td>
                </tr>
                <?php
			  }
			  ?>
